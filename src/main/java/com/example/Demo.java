...
public class Demo extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
    ...
    public static void main(String[] args) {
        ...
        demo.privateMsg(0, 10001, 2234567819L, "三天统计", 0);
        // 模拟群聊消息
        // 开始模拟群聊消息
//        demo.groupMsg(0, 10006, 3456789012L, 3333333334L, "", "[CQ:at,qq=3615427102] 显示：定西市公共资源交易中心", 0);
        // ......
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        ...
        if (msg.contains("屏蔽")) {
            msg = msg.replace("屏蔽", "");
            di.hideMsg(msg);
            CQ.sendPrivateMsg(fromQQ, "屏蔽网站“" + msg + "”成功");
        }
        if (msg.equals("三天统计")) {

            msg = di.al().toString().replace(", ", "\n").replace("[", "").replace("]", "");
            int n = di.al().size();
            CQ.sendPrivateMsg(fromQQ, "近三天省市级公共资源网爬虫数量均为0的爬虫汇总：\n" + msg + "\n共计：" + n + "个网站");
        }

        if (msg.contains("显示")) {
            msg = msg.replace("显示", "");
            di.showMsg(msg);
            CQ.sendPrivateMsg(fromQQ, "显示网站“" + msg + "”成功");
        }
        return MSG_IGNORE;
    ...
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ, String fromAnonymous, String msg,
        ...
        if (msg.contains(CC.at(3615427102L))) {
            DataImportOther di = new DataImportOther();
            if (msg.contains("三天统计")) {
                int n = di.al().size();
                msg = di.al().toString().replace(", ", "\n").replace("[", "").replace("]", "");
                CQ.sendGroupMsg(fromGroup, "近三天省市级公共资源网爬取数量均为0的爬虫汇总：\n" + msg + "\n共计：" + n + "个网站");
            }
        }
        return MSG_IGNORE;
    }
